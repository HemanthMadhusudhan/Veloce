...
print("Initializing IS-Net (InSPyReNet High Resolution Neural Session)...", flush=True)
...
def process_perfect_asset(raw_url, out_path, target_size=800):
    img = download_image(raw_url)
    
    # Check if image is already a transparent PNG (like F1 tees)
    alpha = np.array(img.split()[-1])
    has_transparency = np.mean(alpha < 50) > 0.05
    
    if has_transparency:
        # Native transparent PNG: preserve original crisp alpha
        segmented = img
    else:
        # Run IS-Net for full sleeve & high-resolution contour retention
        segmented = remove(session=isnet_session, data=img)
        
        if is_model_photo(img, segmented):
            segmented = img
        else:
            # Hole-filling protection to ensure solid fabric
...
def main():
    ...
    for i, p in enumerate(all_products):
        ...
        sys.stdout.write(f"[{i+1}/{len(all_products)}] IS-Net processing {safe_name}... ")
        sys.stdout.flush()
        ...
        for img_idx, raw_url in enumerate(raw_images):
            ...
            try:
                process_perfect_asset(raw_url, out_path, 800)
    ...
    print("\n======================================================", flush=True)
    print(f"ALL {len(all_products)} PRODUCTS REBUILT WITH IS-NET HIGH-RESOLUTION SLEEVE & F1 PRESERVATION!", flush=True)
    print("======================================================\n", flush=True)
