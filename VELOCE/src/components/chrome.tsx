    ...
    <nav className="fixed inset-x-0 top-12 z-50 flex justify-center px-4 sm:top-14">
      ...
      >
        ...
        <div className="flex items-center gap-1 sm:gap-2">
          <IconChip onClick={openSearch} label="Search">
            <Search className="h-4 w-4" />
          </IconChip>
          {isAdmin ? (
            <>
              <Link to="/admin" className="hidden sm:block">
                <IconChip label="Admin">
            ...
            <>
              <Link to="/wishlist" className="relative">
                <IconChip label="Wishlist">
                  <Heart className="h-4 w-4" />
                </IconChip>
                {wishlist.length > 0 && <Dot>{wishlist.length}</Dot>}
              </Link>
              <Link to={userEmail ? "/profile" : "/login"} className="hidden sm:block">
      ...
      {open && (
        <div className="fixed inset-0 z-[100] bg-background flex flex-col px-5 pt-20 pb-8 overflow-y-auto animate-in fade-in zoom-in-95 md:hidden">
          ...
          <div className="flex flex-col gap-4">
            <div>
              <div className="px-2 py-0.5 text-[9px] font-bold uppercase tracking-[0.28em] text-brand">
                Football
              </div>
...
function Dot({ children }: { children: ReactNode }) {
  return (
    <span className="pointer-events-none absolute right-0 top-0 flex h-4 min-w-4 items-center justify-center rounded-full bg-brand px-1 font-mono text-[9px] font-bold text-foreground">
      {children}
    </span>
  );
}
      ...
      {open && (
        <div className="absolute left-1/2 top-full w-[720px] max-w-[92vw] -translate-x-1/2 pt-3">
          <div className="glass grid grid-cols-3 gap-6 rounded-2xl p-6">
            {LEAGUES.map((l) => (
              <div key={l.league}>
                <div className="mb-2 text-[9px] uppercase tracking-[0.28em] text-brand">
                  {l.league}
                </div>
      ...
      <div className="grid grid-cols-2 gap-3 rounded-2xl border border-border/50 bg-surface/40 p-4 sm:grid-cols-4 sm:gap-6 sm:p-6">
        {perks.map((p) => (
          <div key={p.label} className="flex items-center gap-3">
            ...
            <div className="min-w-0 text-[11px] font-medium uppercase tracking-[0.18em] text-foreground/90 sm:text-[12px]">
              {p.label}
            </div>
          </div>
        ))}
        ...
        <div className="grid gap-10 md:grid-cols-4">
          <div>
            ...
            <p className="mt-4 max-w-xs text-sm text-muted-foreground">
              Elite football jerseys and Formula 1 team merchandise. Engineered precision. Cinematic
              detail.
            </p>
          </div>
          <FooterCol
        ...
        <div className="border-b border-border/60 bg-brand/10 px-6 py-3 text-[11px] uppercase tracking-[0.18em]">
          {freeUnits > 0 ? (
            <div className="flex items-center gap-2 text-brand">
              <Gift className="h-3.5 w-3.5" /> B2G1 applied · {freeUnits} item
              {freeUnits > 1 ? "s" : ""} FREE
            </div>
          ) : (
            <div className="flex items-center gap-2 text-muted-foreground">
              <Gift className="h-3.5 w-3.5" /> Add {Math.max(0, 3 - totals.itemCount)} more · Buy 2
              Get 1 Free
            </div>
          )}
        </div>
                  ...
                  <div className="flex flex-1 min-w-0 flex-col justify-between">
                    <div className="flex justify-between gap-2">
                      <div className="min-w-0">
                        ...
                        {fu > 0 && (
                          <div className="mt-1 inline-flex items-center gap-1 rounded-full bg-brand/20 px-2 py-0.5 text-[9px] uppercase tracking-[0.2em] text-brand">
                      ...
                      <div className="inline-flex items-center rounded-full border border-border/70">
                        ...
                        <button
                          onClick={() => {
                            ...
                            const available =
                              p?.stockBySize?.[item.size] !== undefined
                                ? p.stockBySize[item.size]
                                : (p?.stock ?? 0);
                            updateQty(
                              item.id,
                              item.size,
                              item.color,
                              Math.min(available, item.qty + 1),
                              item.customName,
                              item.customNumber,
                            );
                          }}
        ...
        {lines.length > 0 && (
          <div className="border-t border-border/60 px-6 py-5 space-y-3">
            <div className="space-y-1 text-xs">
              ...
              {discount > 0 && (
                <Row k={`B2G1 (${couponApplied})`} v={`−${formatINR(discount)}`} accent />
              )}
...
function Row({ k, v, accent }: { k: string; v: string; accent?: boolean }) {
  return (
    <div className="flex justify-between">
      ...
      <span className={accent ? "text-brand font-mono" : "font-mono"}>{v}</span>
    </div>
  );
}
    ...
    <div className="fixed inset-0 z-[100]" role="dialog">
      ...
      <div className="absolute inset-x-4 top-24 mx-auto max-w-2xl overflow-hidden rounded-3xl border border-border/60 bg-background shadow-2xl animate-in fade-in slide-in-from-top-4">
        <div className="flex items-center gap-3 border-b border-border/60 px-5 py-4">
          ...
          />
          <kbd className="rounded border border-border/60 px-2 py-0.5 font-mono text-[10px] text-muted-foreground">
            ESC
          </kbd>
        </div>
        <div className="max-h-[60vh] overflow-y-auto p-4">
          {q === "" ? (
            <div className="space-y-5">
              {recent.length > 0 && (
                <div>
                  <div className="mb-2 flex items-center gap-2 text-[10px] uppercase tracking-[0.24em] text-muted-foreground">
