...
export type CartItem = { id: string; qty: number; size: string; color: string; customName?: string; customNumber?: string };
...
type ShopCtx = {
  ...
  placeOrder: (o: Omit<Order, "id" | "createdAt" | "status"> & { status?: OrderStatus }) => Promise<Order>;
...
export function ShopProvider({ children }: { children: ReactNode }) {
  ...
  const updateProfile = useCallback(async (p: Partial<AppUser>) => {
    if (!userId) return;
    try {
      const dbData: any = {};
  ...
  useEffect(() => {
    if (!authLoading && userId) {
      supabase.from("users").update({ cart }).eq("id", userId).then(({ error }) => {
  ...
  useEffect(() => {
    if (!authLoading && userId) {
      supabase.from("users").update({ wishlist }).eq("id", userId).then(({ error }) => {
  ...
  const updateQty = useCallback((id: string, size: string, color: string, qty: number, customName?: string, customNumber?: string) => {
    setCart((prev) => prev.map((x) => (
      x.id === id && x.size === size && x.color === color && x.customName === customName && x.customNumber === customNumber
        ? { ...x, qty: Math.max(1, qty) } 
        : x
    )));
  }, [setCart]);

  const removeFromCart = useCallback((id: string, size: string, color: string, customName?: string, customNumber?: string) => {
  ...
  const placeOrder = useCallback(async (o: Omit<Order, "id" | "createdAt" | "status"> & { status?: OrderStatus }) => {
  ...
  return (
    <Ctx.Provider
      value={{
        cart, cartOpen, searchOpen, wishlist, recent,
        isAdmin, userEmail, userId, authLoading,
        orders, profile, updateProfile, placeOrder, updateOrderStatus, removeOrder,
        signOut,
