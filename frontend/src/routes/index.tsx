...
import { ArrowUpRight, Play, Gift, X } from "lucide-react";
...
function Home() {
  ...
  return (
    <>
      ...
      <div className="sm:hidden -mt-[76px]">
        ...
        <section className="relative w-full h-[300px] overflow-hidden">
          ...
          <div className="relative z-10 mx-auto flex h-full flex-col justify-end px-4 pb-8 pt-20">
            <div className="animate-reveal">
              <div className="mb-2 flex items-center gap-2 text-[8px] uppercase tracking-[0.2em] text-brand">
                <span className="h-px w-4 bg-brand" /> Season 25 · Global Drop
              </div>
              <h1 className="font-display text-2xl font-bold leading-[0.95] tracking-tight text-foreground">
                Precision.<br/>Rendered <span className="italic text-brand">in fabric.</span>
              </h1>
              <div className="mt-2 inline-flex max-w-full items-center gap-1.5 rounded-full border border-brand/40 bg-brand/10 px-2.5 py-1 text-[8px] uppercase tracking-[0.15em] text-brand">
                <Gift className="h-2.5 w-2.5 shrink-0" /> <span className="truncate">BUY 2 GET 1 FREE · CODE B2G1</span>
              </div>
              <div className="mt-3 flex flex-wrap items-center gap-2">
                <Link to="/shop" className="group inline-flex items-center gap-1.5 rounded-full bg-foreground px-4 py-2 text-[9px] font-semibold uppercase tracking-[0.15em] text-background transition hover:bg-brand hover:text-foreground">
          ...
          {/* ticker at bottom */}
          <div className="absolute bottom-0 left-0 right-0 z-10 border-t border-white/10 bg-background/40 backdrop-blur">
            <div className="flex items-center gap-4 overflow-hidden px-4 py-2 font-mono text-[8px] uppercase tracking-[0.2em] text-muted-foreground">
              <span className="flex items-center gap-1.5"><span className="h-1.5 w-1.5 animate-pulse rounded-full bg-brand" /> LIVE · Monza Drop</span>
            </div>
          </div>
        </section>

        <section className="relative w-full h-[250px] overflow-hidden bg-surface mt-1">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-end px-5 pb-6 text-center z-10">
            <h1 className="font-display text-[1.75rem] leading-[1] font-bold tracking-tight text-white drop-shadow-md mb-4 uppercase">
              FIFA World Cup 2026
            </h1>
            <Link to="/shop/worldcup" className="inline-flex items-center justify-center rounded-full bg-foreground px-6 py-2.5 text-[11px] font-bold uppercase tracking-[0.2em] text-background active:scale-95 transition-transform shadow-lg">
              Shop World Cup
            </Link>
          </div>
        </section>
        ...
        <section className="w-full grid grid-cols-2 gap-1 bg-background pt-1">
           <Link to="/shop/football" className="relative block w-full aspect-square overflow-hidden">
             {isVideoUrl(catFootball) ? (
               <video src={catFootball} autoPlay loop muted playsInline className="absolute inset-0 h-full w-full object-cover opacity-80" />
             ) : catFootball ? (
               <img src={catFootball} alt="Football" className="absolute inset-0 h-full w-full object-cover opacity-80" />
             ) : null}
             <div className="absolute inset-0 bg-black/40" />
             <div className="absolute inset-0 flex items-center justify-center p-3 text-center">
               <h2 className="font-display text-2xl sm:text-3xl font-bold tracking-widest text-white drop-shadow-md">FOOTBALL</h2>
             </div>
           </Link>
           <Link to="/shop/f1" className="relative block w-full aspect-square overflow-hidden">
             {isVideoUrl(catF1) ? (
               <video src={catF1} autoPlay loop muted playsInline className="absolute inset-0 h-full w-full object-cover opacity-80" />
             ) : catF1 ? (
               <img src={catF1} alt="Formula 1" className="absolute inset-0 h-full w-full object-cover opacity-80" />
             ) : null}
             <div className="absolute inset-0 bg-black/40" />
             <div className="absolute inset-0 flex items-center justify-center p-3 text-center">
               <h2 className="font-display text-2xl sm:text-3xl font-bold tracking-widest text-white drop-shadow-md">FORMULA 1</h2>
             </div>
           </Link>
        </section>
        ...
        <section className="w-full py-8 overflow-hidden">
          ...
          <div className="grid grid-cols-2 gap-3 px-4 pb-4">
            {ZONES.map((z) => {
              ...
              return (
                <Link key={z.slug} to="/zone/$slug" params={{ slug: z.slug }} className="group relative block aspect-[4/5] overflow-hidden rounded-xl border border-border/40 shadow-sm">
                  {isVid ? (
                    <video src={img} autoPlay loop muted playsInline className="absolute inset-0 h-full w-full object-cover transition-transform duration-700 group-hover:scale-105" />
                  ) : img ? (
                  ...
                  <div className="absolute inset-x-0 bottom-0 p-3">
                    <span className="block text-[11px] font-bold tracking-widest text-white uppercase truncate">{z.name}</span>
                    <span className="block mt-0.5 text-[9px] text-white/70 uppercase tracking-widest truncate">{z.category === 'f1' ? 'The Paddock' : 'The Pitch'}</span>
                  </div>
                </Link>
              );
            })}
      ...
      <section className="relative -mt-24 hidden sm:block h-[100svh] min-h-[700px] w-full overflow-hidden">
        ...
        {/* ticker at bottom */}
        <div className="absolute bottom-0 left-0 right-0 z-10 border-t border-white/10 bg-background/40 backdrop-blur">
          <div className="flex items-center gap-6 overflow-hidden px-4 py-2.5 font-mono text-[9px] uppercase tracking-[0.22em] text-muted-foreground sm:gap-8 sm:px-6 sm:py-3 sm:text-[10px] sm:tracking-[0.28em]">
            <span className="flex items-center gap-2"><span className="h-1.5 w-1.5 animate-pulse rounded-full bg-brand" /> LIVE · Monza GP Drop</span>
            <span className="hidden sm:inline">FREE SHIPPING OVER ₹499</span>
            <span className="hidden md:inline">Lifetime authentication</span>
          </div>
        </div>
      </section>
      ...
      {/* PLAYER ZONES */}
      <section className="mx-auto mt-12 hidden sm:block max-w-7xl px-5 sm:mt-24 sm:px-6">
        <div className="mb-6 sm:mb-8 flex items-end justify-between">
          <div>
            <div className="text-[9px] uppercase tracking-[0.28em] text-brand sm:text-[10px]">Icons · Curated capsules</div>
            <h2 className="mt-2 font-display text-2xl font-bold sm:text-5xl">Player Zones.</h2>
          </div>
        </div>
      ...
      {/* FEATURED PRODUCTS */}
      <section className="mx-auto mt-12 max-w-7xl px-5 sm:mt-20 sm:px-6">
        <div className="mb-6 sm:mb-10 flex items-end justify-between">
          <div>
      ...
      {/* CUSTOMER REVIEWS */}
      <section className="mx-auto mt-16 max-w-7xl px-5 sm:mt-24 sm:px-6">
